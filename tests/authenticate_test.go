...
import (
	...
	"github.com/jsfelipearaujo/lambda-login/src/database"
	"github.com/jsfelipearaujo/lambda-login/src/handlers"
	"github.com/jsfelipearaujo/lambda-login/src/hashs"
	"github.com/jsfelipearaujo/lambda-login/src/providers"
	"github.com/jsfelipearaujo/lambda-login/src/token"
...
func (af *appFeature) theUserRequestToBeAuthenticated(ctx context.Context) (context.Context, error) {
	...
	resp, err := handler.AuthenticateUser(req)
	if err != nil {
		return ctx, err
	}
...
func (af *appFeature) theUserShouldBeAuthenticatedSuccessfully(ctx context.Context) (context.Context, error) {
	responseStatus := getResponseStatus(ctx)
	if responseStatus != http.StatusCreated {
		return ctx, fmt.Errorf("expected status code 201, but got %d", responseStatus)
	}
...
func InitializeScenario(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^the user request to be authenticated$$`, app.theUserRequestToBeAuthenticated)
	ctx.Step(`^the user should be authenticated successfully$`, app.theUserShouldBeAuthenticatedSuccessfully)
